...
df_r = pd.read_csv('E:/colordetectanalysis/JAI_D50_TC918_220_2.csv', header=None) #reference
df_t = pd.read_csv('E:/colordetectanalysis/JAI_HRa_mixedLED_TC918_220_2.csv', header=None) #target

final_image = np.zeros((1160, 3203, 3), np.uint8)
#根据CSV文档数据生成的张彩色图片（28*40，20*40）
color_re = np.zeros((1120, 800, 3), np.uint8) #reference光源图片
color_tr = np.zeros((1120, 800, 3), np.uint8) #target光源图片
...
delta_delta_gray[0:40*2, 0:40*20] = color_tr[0:40*2, 0:40*20] #前两行和target的前两行色块相同。
for i in range(2, 28):
    for j in range(0, 20):
        ...
        if np.argmax(abs(line_delta_array[i-2, j])) == 0:
            cv2.putText(delta_delta_gray, str(round(line_delta_array[i-2, j, 0],2)), (40*j+2, 40*i+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5,  (255, 0, 0), 1)
        elif np.argmax(abs(line_delta_array[i-2, j])) == 1:
            cv2.putText(delta_delta_gray, str(round(line_delta_array[i-2, j, 1],2)), (40*j+2, 40*i+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5,  (0, 255, 0), 1)
        elif np.argmax(abs(line_delta_array[i-2, j])) == 2:
            cv2.putText(delta_delta_gray, str(round(line_delta_array[i-2, j, 2],2)), (40*j+2, 40*i+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5,  (0, 0, 255), 1)

delta_E = cv2.imread("C:/Users/t03492/Desktop/color_analysis_pictures/colorBoard_show/TC918_1_dE_onHraImage_JAI.bmp")
...
final_image[40:1160, 2403:3203] = delta_delta_gray

for picNum in range(0,4): 
    for j in range(0, 20):
        cv2.putText(final_image, str(j+1), (801*picNum+40*j+5, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8,  (10, 10, 10), 2)

cv2.imwrite('C:/Users/t03492/Desktop/color_analysis_pictures/deltaE_show/JAI_D50&HRamixedLED_TC918_220_2_g.bmp', final_image )
